...
describe("ArrayHelper", () => {
  ...
  describe('takeLeft and takeRight', () => {
    test('should take from left', () => {
      const left = ArrayHelper.takeLeft(2)([1, 2, 3]) // [1, 2]
      expect(left).toEqual([1, 2])
    })

    test('should take from right', () => {
      const right = ArrayHelper.takeRight(2)([1, 2, 3, 4, 5]) // [4, 5]
  ...
  describe('drop right', () => {
    test('should drop to right', () => {
      const arr = [1, 2, 3, 4, 5]
      const output = ArrayHelper.dropRight(arr, 3)
  ...
  describe('dropLeftWhile', () => {
    test('should drop left while predicate returns true', () => {
      const r = ArrayHelper.dropLeftWhile((n: number) => n % 2 === 1)([1, 3, 2, 4, 5])

      expect(r).toEqual([2, 4, 5])
    })
    test('should drop right while predicate returns true', () => {
      const greaterThan2 = ArrayHelper.dropRightWhile((n: number) => n > 2)([1, 3, 2, 4, 5])
      expect(greaterThan2).toEqual([1, 3, 2])
    })
  })
  ...
  describe('first and last', () => {
    test('should return first', () => {
      const first = ArrayHelper.first([2, 5, 4])
      expect(first).toBe(2)
    })
    test('should return last', () => {
      const first = ArrayHelper.last([2, 5, 4])
      expect(first).toBe(4)
      const none = ArrayHelper.last([])
